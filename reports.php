        ...
        <div class="card card-outline card-success">
          <div class="card-header">
           <form id = "form_user" method = "POST" enctype = "multi-part/form-data">
              <!--<label for="exampleInputEmail1">Start From Date:</label>
              <input type="date"  autocomplete="off" name="from" >

              <label for="exampleInputEmail1">To:</label>
              <input type="date"  autocomplete="off" name="to" >
...
-->
            ...
            <div class="card-tools">
            	...
            	<a href="pdfl.php"  class="btn btn-flat btn-sm bg-gradient-success btn-success" > <i class="fa fa-print"></i>Print </a>
            </div>
          </div>
              ...
              <table class="table m-0 table-bordered">
               <!--  <colgroup>
                ...
                if($_SESSION['login_type'] == 2){
                  $where = " where manager_id = '{$_SESSION['login_id']}' ".$sql;
                }elseif($_SESSION['login_type'] == 3){
                  $where = " where concat('[',REPLACE(user_ids,',','],['),']') LIKE '%[{$_SESSION['login_id']}]%' ".$sql;
                }
                $sqq="SELECT * FROM project_list $where order by name asc";
                var_dump($sqq);
                $qry = $conn->query($sqq);
                $_SESSION['sql']=$sqq;
